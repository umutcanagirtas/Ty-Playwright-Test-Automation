...
test.describe('Lighthouse Performance Test', () => {
  ...
  test('@test @t1', async ({ page }) => {
    await runPerformanceTest(page, '/', 'Home Page', desktopConfig);
  });

  test('@test @t2', async ({ page }) => {
    await runPerformanceTest(page, '/about', 'Desktop - About Page', desktopConfig);
  });
